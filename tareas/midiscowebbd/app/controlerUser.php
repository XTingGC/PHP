...
function  ctlUserInicio(){
    ...
    if ( $_SERVER['REQUEST_METHOD'] == "POST"){
        if (isset($_POST['user']) && isset($_POST['clave'])){
            $user =$_POST['user'];
            $clave=$_POST['clave'];
            if ( modeloUser::modeloOkUser($user,$clave)){
                $_SESSION['user'] = $user;
                $_SESSION['tipouser'] = modeloUser::modeloObtenerTipo($user);
...
function ctlUserAlta (){
    ...
    if ( $_SERVER['REQUEST_METHOD'] == "POST"){
        ...
        $plan   = $_POST['plan'];
        $estado = $_POST['estado'];
        $msg = modeloUser::modeloErrorValoresAlta($user, $clave1, $clave2, $nombre, $email, $plan, $estado);
        if ( !$msg ){
            if ( modeloUser::modeloUserAdd($user, [$clave1,$nombre,$email,$plan,$estado]) ){
 ...
 */
//es cuando pincha modificar y luego, yuna vez  modificado, cuando guardas.
function ctlUserModificar (){
    
    if ( $_SERVER['REQUEST_METHOD'] == "GET"){
        $datosuser = modeloUser::modeloUserGet($_GET['userid']);
        //var_dump($datosuser);
        $user=$_GET['userid'];
        $clave1=$datosuser[0];
        $clave2=$datosuser[0];
    ...
    if ( $_SERVER['REQUEST_METHOD'] == "POST"){
        ...
        if (empty($_POST['user'])  || empty($_POST['nombre']) ||
            ...
            $msg = modeloUser::modeloErrorValoresModificar($user, $clave1, $clave2, $nombre, $email, $plan, $estado);
            if ( !$msg ){
                if ( modeloUser::modeloUserUpdate($user, [$clave1,$nombre,$email,$plan,$estado]) ){
...
function ctlUserBorrar(){
    ...
    if ( modeloUser::modeloUserDel($userid)){
...
function ctlUserVerUsuarios (){
    ...
    $usuarios = modeloUser::modeloUserGetAll(); 
    // Invoco la vista 
